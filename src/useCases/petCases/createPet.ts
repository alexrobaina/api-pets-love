...
export const create = async (req: Request, res: Response) => {
  try {
    ...
    if (res.locals.file?.newImages?.url) cleanedData.images = [res.locals.file.newImages?.url];
    if (res.locals.file?.newImages?.urls) cleanedData.images = res.locals.file.newImages?.urls;
...
const generateQRCodeDataURL = async (text: string) => {
  try {
    const options = {
      ...
      scale: 100, // Large scale to create a high-resolution image
      width: 1000,
      height: 1000,
      color: {
