...
const petProps =
  'age name city medicalNotes gender color images country adopted category createdDate location textAddress updatedDate userCreator weight description vet userAdopted userVet';
...
export const save = async (body: any, images: any) => {
  const petBody = {
    age: body.age,
    name: body.name,
    city: body.city,
  ...
  if (body?.userAdopted !== 'null') {
    // @ts-ignore
    petBody.userAdopted = body?.userAdopted;
  }

  if (body?.userVet !== 'null') {
    // @ts-ignore
    petBody.userVet = body?.userVet;
  }

  try {
    const pet = new Pet(petBody);
    await pet.save();
